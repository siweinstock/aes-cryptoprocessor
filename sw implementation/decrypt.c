...
void inv_shift_rows(uint8_t* state) {
    ...
    for (j=0; j<4; j++) {
        for (i = 0; i < 4; i++) {
            tmp[4 * j + i] = state[(4 * j + 13 * i) % BLK_SIZE];
        }
    }
...
void inv_mix_columns(uint8_t* state) {
    ...
    for (j=0; j<4; j++) {
        for (i = 0; i < 4; i++) {
            tmp[i+4*j] = mul14[state[(i % 4)+4*j]] ^ mul11[state[(i + 1) % 4+4*j]] ^ mul13[state[(i + 2) % 4+4*j]] ^ mul9[state[(i + 3) % 4+4*j]];
        }
    }
    memcpy(state, tmp, BLK_SIZE* sizeof(uint8_t));
}
